...
class lap_qd_ktra_hoan_gtgt:
    ...
    def to_trinh(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "3.to_trinh_ktr_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        # ghi dữ liệu thành phần đoàn
        table = document.tables[0]
        # del_row(table, len(doan_ktra['<ten_cb>']), len(table.rows))
    ...
    def qd_gsat(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "3.qd_giam_sat_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_QD_giam_sat.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store/" + filename)   
        document.save(path)
    ...
    def kh_gsat(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "3.kh_giam_sat_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_KH_giam_sat.docx"
        # path = os.path.join(settings.STATICFILES_DIRS[0], "media_store/" + filename)
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)
    ...
    def qd_ktra(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "3.qd_ktra_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        # ghi dữ liệu thành phần đoàn
        table = document.tables[0]
        # del_row(table, len(doan_ktra['<ten_cb>']), len(table.rows))
...
#########################################################################################
#########################################################################################
class lap_qd_ktra_sau_hoan_gtgt:
    def __init__(self, tt_qd, doan_ktra):
            self.tt_qd = tt_qd
            self.doan_ktra = doan_ktra
    ...
    def to_trinh(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "8.to_trinh_ktr_sau_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        # ghi dữ liệu thành phần đoàn
        table = document.tables[0]
        # del_row(table, len(doan_ktra['<ten_cb>']), len(table.rows))
    ...
    def qd_gsat(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "8.qd_giam_sat_sau_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_QD_giam_sat.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store/" + filename)   
        document.save(path)
    ...
    def kh_gsat(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "8.kh_giam_sat_sau_hoan_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_KH_giam_sat.docx"
        # path = os.path.join(settings.STATICFILES_DIRS[0], "media_store/" + filename)
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)
    ...
    def qd_ktra(self):
        ...
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "8.qd_ktra_hoan_sau_gtgt_HT.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        # ghi dữ liệu thành phần đoàn
        table = document.tables[0]
        # del_row(table, len(doan_ktra['<ten_cb>']), len(table.rows))
...
#########################################################################################
#########################################################################################
class lap_qd_ktra_dot_xuat:
    def __init__(self, tt_qd, doan_ktra):
        self.tt_qd = tt_qd
        self.doan_ktra = doan_ktra

    def to_trinh(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "5.to_trinh_ktr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_To_trinh.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)      
        document.save(path)
    ...
    def qd_gsat(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "5.qd_giam_sat_ktr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_QD_giam_sat.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)
        document.save(path)
    ...
    def kh_gsat(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "5.kh_giam_sat_ktr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_KH_giam_sat.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)
        document.save(path)
    ...
    def qd_ktra(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "5.qd_ktr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        # ghi dữ liệu thành phần đoàn
        table = document.tables[0]
        for i in range(len(self.doan_ktra['<ten_cb>'])):
...
#########################################################################################
#########################################################################################
class lap_qd_ttra_dot_xuat:
    def __init__(self, tt_qd, doan_ttra):
        self.tt_qd = tt_qd
        self.doan_ttra = doan_ttra

    def to_trinh(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "6.to_trinh_ttr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_To_trinh.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)      
        document.save(path)
    ...
    def qd_gsat(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "6.qd_giam_sat_ttr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_QD_giam_sat.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)
        document.save(path)
    ...
    def kh_gsat(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "6.kh_giam_sat_ttr_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        filename = self.tt_qd["<mst>"] + "_KH_giam_sat.docx"
        path = os.path.join(settings.STATICFILES_DIRS[0], "media_store", filename)
        document.save(path)
    ...
    def qd_ttra(self):
        document = Document(os.path.join(settings.STATICFILES_DIRS[0], "media", "6.qd_ttra_dot_xuat.docx"))
        ghi_du_lieu_para(document, self.tt_qd)
        # ghi dữ liệu thành phần đoàn
        table = document.tables[0]
        for i in range(len(self.doan_ttra['<ten_cb>'])):
