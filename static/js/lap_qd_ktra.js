$(document).ready(function () {
    new Cleave('.ngay_ktra', {
        date: true,
        delimiter: '/',
        datePattern: ['d','m','Y']
    });
    new Cleave('.ngay_thang', {
        date: true,
        delimiter: '/',
        datePattern: ['m','Y']
    });
    // jQuery button click event to add a row.
    var rowIdx = 0; 
    $('#addBtn').on('click', function () {
    // Adding a row inside the tbody.
        $('#tbody').append(
            `<tr id="R${++rowIdx}">
                <td class="row-index">
                        <p class="cb_gioi_tinh"></p></td>
                <td class="row-index">
                    <input class="thanh_vien" name="thanh_vien" type="text" size="24" required></td>
                <td class="row-index">
                        <p class="ngach_cb"></p></td>
                <td class="row-index">
                        <p class="cv_doan"></p></td>
                <td class="text-center">
                    <button class="btn btn-danger remove" 
                        type="button">Xóa</button></td>
            </tr>`);
        $('input.thanh_vien').autocomplete({
            minLength: 2,
            source : 'cb_ten_autocomplete',
        });
        // Thành viên đoàn kiểm tra
        $('.thanh_vien').each(function () {
            $(this).change(function () {
                var thanh_vien = $(this);
                $.ajax({
                    url: 'cb_thong_tin',
                    data: {'ten_cb' : $(this).val().trim()},
                    dataType: 'json',
                    success: function (data) {
                        if ($.isEmptyObject(data)) {
                            alert("Cán bộ không có tên trong danh sách\nVui lòng cập nhật");
                            thanh_vien.val('');   
                        } else {  
                            var tv =  thanh_vien.closest('tr');
                            tv.find('.cb_gioi_tinh').text(data.gioi_tinh);      
                            tv.find('.ngach_cb').text(data.ngach_cb);   
                            if (tv.attr('id') == 'R1') {
                                tv.find('.cv_doan').text("Trưởng đoàn"); 
                            } else {
                                tv.find('.cv_doan').text("Thành viên"); 
                            }
                        }
                    }
                });
            });
        });

        
    });

    $('#tbody').on('click', '.remove', function () {    
        // Getting all the rows next to the 
        // row containing the clicked button
        var child = $(this).closest('tr').nextAll();

        // Iterating across all the rows 
        // obtained to change the index
        child.each(function () {
            
            // Getting <tr> id.
            var id = $(this).attr('id');

            // Getting the <p> inside the .row-index class.
            //var idx = $(this).children('.row-index').children('p');

            // Gets the row number from <tr> id.
            var dig = parseInt(id.substring(1));

            // Modifying row index.
            //idx.html(`Row ${dig - 1}`);

            // Modifying row id.
            $(this).attr('id', `R${dig - 1}`);
        });

        // Removing the current row.
        $(this).closest('tr').remove();

        // Decreasing the total number of rows by 1.
        rowIdx--;
    });
    // Hiển thị thông tin nnt
    $('.mst').change (function () {
        var mst = $('.mst').val().trim();
        $.ajax({
            url: 'nnt_thong_tin',
            data: {'mst' : mst},
            dataType: 'json',
            success: function (data) {
                if($.isEmptyObject(data)) {
                    alert("Người nộp thuế không tồn tại trong danh bạ\nVui lòng cập nhật");
                    $('.mst').val('');    
                } else {               
                    $('.ten_dv').text(data.ten_nnt);
                    $('.dia_chi').text("Địa chỉ: " + data.dia_chi);
                }
            } 
        });
    });  

    //Hiển thị thông tin cán bộ
    $('.ng_giam_sat').change(function () {
        var ten_cb = $('.ng_giam_sat').val().trim();
        $.ajax({
            url: 'cb_thong_tin',
            data: {'ten_cb' : ten_cb},
            dataType: 'json',
            success: function (data) {
                if($.isEmptyObject(data)) {
                    alert("Cán bộ không có tên trong danh sách\nVui lòng cập nhật");
                    $('.ng_giam_sat').val('');   
                } else if (data.chuc_vu == "Công chức") { 
                    alert("Công chức không được quyền giám sát hoạt động của Đoàn kiểm tra");
                    $('.ng_giam_sat').val('');
                    $('.giam_sat_gioi_tinh').empty();
                    $('.ng_giam_sat_cv').empty();
                    $('.ct').empty();
                } else {
                    $('.giam_sat_gioi_tinh').text(data.gioi_tinh.toLowerCase());                
                    $('.ct').text('Cục Thuế tỉnh Quảng Trị ');
                    if (data.chuc_vu.includes("Cục trưởng")) {
                        $('.ng_giam_sat_cv').text(" - " + data.chuc_vu);
                    } else {
                        $('.ng_giam_sat_cv').text(" - " + data.chuc_vu + " Thanh tra - Kiểm tra");
                    }               
                }
            }
        });
    });
    // Phê duyệt tờ trình
    $('.ten_ld_to_trinh').change(function () {
        var ten_cb = $('.ten_ld_to_trinh').val().trim();
        $.ajax({
            url: 'cb_thong_tin',
            data: {'ten_cb' : ten_cb},
            dataType: 'json',
            success: function (data) {
                if($.isEmptyObject(data)) {
                    alert("Lãnh đạo không có tên trong danh sách\nVui lòng cập nhật");
                    $('.ten_ld_to_trinh').val('');   
                } else {
                        $('.ld_to_trinh').text(data.chuc_vu);               
                }          
            }
        });
    });
    // Lãnh đạo cơ quan
    $('.ten_ld_qd').change(function () {
        var ten_cb = $('.ten_ld_qd').val().trim();
        $.ajax({
            url: 'cb_thong_tin',
            data: {'ten_cb' : ten_cb},
            dataType: 'json',
            success: function (data) {
                if($.isEmptyObject(data)) {
                    alert("Lãnh đạo không có tên trong danh sách\nVui lòng cập nhật");
                    $('.ten_ld_qd').empty();
                    $('.hinh_thuc_ky').empty();
                } else {
                    switch (data.chuc_vu) {
                        case "Cục trưởng":
                            $('.hinh_thuc_ky').empty();
                            $('.lq_qd').html(data.chuc_vu.toUpperCase());
                            break;
                        case "Phó Cục trưởng":
                            $('.hinh_thuc_ky').html("KT.CỤC TRƯỞNG");
                            $('.lq_qd').html(data.chuc_vu.toUpperCase());
                            break;
                        default:
                            alert("Không có thẩm quyền phê duyệt quyết định\nVui lòng kiểm tra lại");
                            $('.ten_ld_qd').val('');
                            $('.hinh_thuc_ky').empty(); 
                            $('.lq_qd').empty();
                    }
                }    
            }
        });
    });

    /* function leading_zero (str, max) {
    str = parseInt(str);
    return str < max ? ("0" + str) : str.toString();
    } */

    $(".mst").autocomplete({
        minLength: 3,
        //source: mst
        source: 'mst_autocomplete',
    });
    $(".ng_giam_sat, .ten_ld_to_trinh, .ten_ld_qd").autocomplete({
        minLength: 2,
        source: 'cb_ten_autocomplete'
    });
    /* var i = 0;
    var tencb = []; var ngachcb = []; var cvdoan = [];
    $('#tbody tr').each(function() {
        tencb[i] = $(this).find(".cb_gioi_tinh").html() + ": " + $(this).find(".thanh_vien").val();
        ngachcb[i] = $(this).find(".ngach_cb").html(); 
        cvdoan[i] = $(this).find(".cv_doan").html(); 
        ++i;
    }); */
});